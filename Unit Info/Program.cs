...
namespace Unit_Info
{
    class Program
    {
        ...
        async static Task Main(string[] args)
        {
            ...
            // await program.ScrapeTest_UnitHandbook();
            // await program.ScrapeTest_Course();

            // Stopwatch stopwatch = new Stopwatch();
            // stopwatch.Start();
            // await program.Scrape_Units(2021, 3000);
            // stopwatch.Stop();
            // Console.WriteLine("Scrape took: {0}", stopwatch.Elapsed.TotalSeconds);

            //await program.ProcessAllCourses();
        ...
        {
            var unitApiRequest = new UnitApiRequestBuilder() { ImplementationYear = 2021 };
            var jsonString = await MacquarieHandbook.DownloadString(unitApiRequest);
            
            // MacquarieDataResponseCollection<MacquarieUnit> unitCollection = JsonConvert.DeserializeObject<MacquarieDataResponseCollection<MacquarieUnit>>(jsonString);
            
            var unitCollection = await MacquarieHandbook.GetDataResponseCollection<Macquarie.Handbook.Data.MacquarieUnit>(unitApiRequest);
            
            MacquarieUnit unit = unitCollection[0];
            unit.DeserialiseInnerJson();
            Console.WriteLine(unit.UnitData.Level.Value);
        }

        public class Temp {
            [JsonProperty("contentlets")]
            public List<MacquarieUnit> units { get; set; }
        }

        public class Test
        {
            MacquarieCourseHandbook.MacquarieCourse contentlets { get; set; }
        }
